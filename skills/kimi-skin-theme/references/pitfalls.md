...
- **硬规则：写任何组件规则前，必须先在活页面 probe 到目标元素（真实类名 + 计算样式），把结果贴进迭代记录；probe 不到就标注"未验证"，禁止凭同类页面的经验类推。** 同一功能的组件在不同页面可能类名不同（插件市场是 `.plugin-use-btn`，技能页是 SkillsView 的 `.card-btn`）——猜到不存在的类名，规则写了等于没写。
- token 重映射会改变全局继承文字色：所有**原生深色组件**（原生 `var(--Colors-KMBlue)` 底的按钮等）都要逐一排查成对声明，不只是主题自己新增的深色块。
- 元素不一定在你以为的容器里：悬浮控件可能是 `.page` 的直接子级（如 `.sidebar-expand-btn` 不在 `aside.sidebar` 内），作用域规则会漏掉它，probe 时沿祖先链确认归属。
- 每轮改完跑运行时审计 `scripts/probe-contrast.mjs <cdp端口>`（WCAG 对比度，含 token 层叠和半透明合成的有效底色），代替肉眼巡检小元素。
