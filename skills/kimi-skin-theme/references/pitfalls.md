...
- 新增或不确定的原生组件选择器，写 CSS 前必须在活页面确认元素存在、类名、祖先归属和关键计算样式。同一功能的组件在不同页面可能类名不同（插件市场是 `.plugin-use-btn`，技能页是 SkillsView 的 `.card-btn`）。兼容清单、模板或本轮已经确认过的选择器可以复用；无法打开对应页面时标记“未验证”，禁止用宽泛的 `[class*="..."]` 规则兜底。
...
- 可选运行 `scripts/probe-contrast.mjs <cdp端口>` 辅助定位低对比度元素。它对半透明背景做近似合成，渐变、图片、滤镜和元素整体透明度仍需结合截图与计算样式确认；不得用脚本结果代替视觉检查。
...
- 不猜新增或不确定的选择器。先用现有探测工具或临时只读探测确认元素存在、类名、祖先归属和计算样式，再写 CSS；最终报告列出已验证选择器和未检查页面。
- 根据问题粒度选择修改范围：局部问题定向修正，视觉方向不成立时允许整体重构。改完跑 `validate` + `check-theme`，等热重载后截图复核。
